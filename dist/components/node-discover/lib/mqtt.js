...
function MqttHandler(mqttopts, network) {
  ...
  this.discoverHostsTimer = setTimeout(function () {
    that.discoverHosts();
  }, this.opts.host_resolve_initial_delay); // console.log('Starting mqtt with ',this.opts,'and',this.network.getInstanceUuid(), '/', this.network.getProcessUuid() );

  this.hosts = {};
}

util.inherits(MqttHandler, EventEmitter); // MqttHandler.setMaxListeners(20);
    ...
    for (i in records) {
      if (typeof that.hosts[records[i]] == 'undefined') {
        ...
        that.hosts[records[i]].on('message', function (id, message, mqtthostid) {
          ...
          if (message.length === 0) {
            // console.log('!!!!!!!!!!!!!!!!!!!!!!!!!');
            that.emit('message', id, '', {
              port: that.port
            }, mqtthostid);
          } else {
            if (that.network.key !== null) {
              that.emit('message', id, Buffer.from(message, 'base64').toString(), {
                port: that.port
              }, mqtthostid);
            } else {
              that.emit('message', id, message.toString(), {
                port: that.port
              }, mqtthostid);
            }
          }
...
MqttHandler.prototype.send = function (msg, offset, length, port, address) {
  ...
  if (that.lastSend != msg) {
    ...
    for (i in that.hosts) {
      setImmediate(function (message, index) {
