...
function MqttHost(ip, mqttnetwork) {
  ...
  this.sent_last = ''; // console.log( 'Starting host connection to', this.ip );
  ...
  that.client.on('close', function () {
    that.emit('mqtthostdisconnected', that.ip); // console.log('disconnected close @',that.ip);
  });
  that.client.on('offline', function () {
    that.emit('mqtthostdisconnected', that.ip); // console.log('disconnected offline @',that.ip);
  });
  that.client.on('end', function () {
    that.emit('mqtthostdisconnected', that.ip); // console.log('disconnected end @',that.ip);
  });
}
...
MqttHost.prototype.send = function (str) {
  ...
  if (that.sent !== str) {
    ...
    that.client.publish(that.announce_topic, str, {
      retain: true
    });
    that.sent = str;
  }
...
//# sourceMappingURL=mqtthost.js.map