...
const util = require('util'); // var reservedEvents = ['promotion', 'demotion', 'added', 'removed', 'master', 'hello'];
...
function Discover(options, callback) {
  ...
  const settings = self.settings = {
    ...
    // default is set at the network layer (true)
  ...
  self.evaluateHello = function (id, data, obj, rinfo, mqtthostid) {
    ...
    self.sources[id][mqtthostid] = 1; // console.log('Adding',id,'- source ',mqtthostid);
    // var wasMaster = null;
    // if (!isNew) {
    ...
    for (const processUuid in self.nodes) {
      ...
      if (node.hostName === hostname) {
        if (typeof self.sources[hostname] !== 'undefined') {
          delete self.sources[hostname][mqtthostid]; // console.log('Removing',hostname,'from source',mqtthostid);
          ...
          if (count == 0) {
            delete self.sources[hostname]; // console.log('Clearing ',hostname,'- no source');
          }
        }
  ...
  self.broadcast.on('mqtthostdisconnected', function (mqtthostid) {
    ...
    for (const processUuid in self.nodes) {
      node = self.nodes[processUuid];
      hostname = node.hostName; // console.log(' in peer',hostname,' ?');
  ...
  self.start = function (callback) {
    ...
    self.broadcast.start(function (err) {
      ...
      running = true; // checkId = setInterval(self.check, checkInterval());
  ...
  self.stop = function () {
    ...
    self.broadcast.stop(); // clearInterval(checkId);
